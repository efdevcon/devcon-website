            ...
            <div key={edition.id} className="clear-bottom clear-top">
              <div className={css['edition']}>
                <div className={css['left']}>
                  {getEditionImage(edition.number) && <Image src={getEditionImage(edition.number)} alt={`${edition.title} event image`} />}
                </div>
...
export async function getStaticProps(context: any) {
  ...
  const editions = GetDevconEditions(context.locale).sort((a, b) => b.number - a.number).filter(i => i.startDate && i.startDate < new Date().getTime())
